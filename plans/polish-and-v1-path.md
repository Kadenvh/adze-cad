...
| R2.5 | **(follow-up)** Task Pane banner wiring — surface `AdzeAddIn.LastProbeMessage` when the probe detected incompatibility so users understand why context-menu is absent. | ✅ 2026-04-19. `HostState.SetProbeResult` / `GetProbeFailure` + `TaskPaneControl.BuildProbeBannerHtml` render a `health-warning` banner above `#chat-area`. Follow-up VHT-20: banner misses initial render, appears after first run (probe completes after the first `BuildFullPageHtml`). |
| R2.6 | **(follow-up)** Live validation — user sets `SOLIDWORKS_AI_CONTEXT_MENU=true` at user scope, relaunches SW, observes expected log entries (probe OK + managed skip, or probe-failed-at-step). Until this lands, R1 is not closed. | ✅ Validated 2026-04-19 — Outcome B. Probe failed at `create-command-group`; ribbon + context-menu gracefully skipped; `ConnectToSW completed` with Task Pane + 10 tools. **`CreateCommandGroup2` confirmed as the broken interop call on SW 34.1.0.0140.** R1 closed. |

### R3 — Update-lifecycle cooperation

| # | Done-when | Status |
|---|-----------|--------|
| R3.1 | **Pre-update eject.** Detect the 3DX desktop updater process (`swxdesktopupdate.exe`) or its file locks; when present, Adze unhooks its SOLIDWORKS event subscriptions on disconnect and skips re-registering until the next clean launch. | ✅ Code 2026-04-19. `AdzeAddIn.HandlePreUpdateEjectIfNeeded` runs at the start of `DisconnectFromSW`, detects `swxdesktopupdate.exe` via `Process.GetProcessesByName`, clears `SwBuildStateService` persisted build, logs a distinctive "Pre-update eject" line. Try/catch-wrapped so failure never blocks normal disconnect. Live validation deferred to R5.5 (full 3DX update cycle). |
...
| R3.4 | Document the update flow in `SETUP.md` and `PRIVACY.md`. | ✅ 2026-04-19. SETUP.md: "Before a SOLIDWORKS or 3DEXPERIENCE update..." callout + Eject-for-Update button guidance. PRIVACY.md: new "Update lifecycle and compatibility state" section describing probe behavior, sw-build.txt persistence, process-detection scope (all local). Also refreshed the API keys section to reflect the DPAPI key store alongside env vars + `.env`. |
...
| R4.5 | `README.md` end-user section rewritten around the Manager UI, not the PowerShell flow. | ✅ 2026-04-19. README.md and SETUP.md both rewritten: end-user path now says the `.bat` "opens the Adze Manager" with explicit Install / Uninstall / Eject for Update / Refresh buttons; PowerShell retained as a headless/CI alternative. Eject for Update explained in both docs. |
| R4.6 | **(follow-up)** Auto-detect `swxdesktopupdate.exe` running — Manager should gray out Install and light up Eject as the prominent action. | ✅ 2026-04-19. `ApplyUpdaterEmphasis` called from `RefreshStatus`: Install disables + grays (`#C8CDD7`); Eject promotes to red (`#DC3545`) with white bold text + `⚠` prefix; status line reads "Eject before the updater applies changes" in red. Reverts on next Refresh when updater exits. |
