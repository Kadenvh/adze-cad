...
| Unit test layer | 503 NUnit 3 compiled tests across broker, tools (read + write + retrieval), trace, learning, memory, cost budgets, feature gates, local provider config, streaming, capability probing, rate limiting, and tool result truncation | Provides fast regression coverage for pure logic without requiring SOLIDWORKS |
