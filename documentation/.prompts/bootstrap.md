...
Bootstrap involves reading a lot of code. Use sub-agents to parallelize the initial analysis:

- Spawn agents to read different directories or modules in parallel during the deep analysis phase.
- For large codebases, spawn agents to catalog different aspects simultaneously: one for file structure mapping, another for dependency analysis, another for configuration and environment setup.
- Keep document creation sequential (ROADMAP → IMPL_PLAN → CLAUDE.md) since each informs the next.
...
- Examine all source files to understand actual implementation — spawn sub-agents to read different directories in parallel for large codebases
...
Target **80–300 lines** for most projects:
